...
TEMPLATES_DIR = os.path.join("DJREACT/templates/ReactDJ",)
...
def generate_fullstack(pName: str, pPath: str) -> None:
    ...
    os.rename(os.path.join(f'{project_path}/Backend/Backend'), os.path.join(f'{project_path}/Backend/{pName}'))

    # Generate SECRET_KEY
    secret_key = secrets.token_urlsafe(50)
    settings_path = os.path.join(f'{project_path}/Backend/{pName}/settings.py')
    ...
    # Create .env file
    env_path = os.path.join(f"{project_path}/Backend", '.env')
    with open(env_path, 'w') as file:
        file.write(f"SECRET_KEY={secret_key}\n")

    print(
        f"\nDjango React boilerplate - '{pName}' created at '{project_path.strip('.')}'.\n"
